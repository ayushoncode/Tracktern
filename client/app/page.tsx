...
import { Rocket, Mail, Lock, Eye, EyeOff, ArrowRight, Users, Zap, BarChart3, Brain, CheckCircle, Star, ChevronLeft, ChevronRight } from "lucide-react"
...
const SCREENSHOTS = [
  ...
  { src: "/screenshots/ai-prep.png", label: "AI Interview Prep" },
  { src: "/screenshots/skill-gap.png", label: "Skill Gap Analysis" },
  { src: "/screenshots/practice.png", label: "Daily Practice" },
  { src: "/screenshots/community.png", label: "Community Experiences" },
]
    ...
    <div className="min-h-screen overflow-x-hidden" style={{background: "#08080f", color: "#e8e8f0"}}>
      ...
      <section className="px-6 pb-20">
        <div className="max-w-5xl mx-auto relative">
          ...
          <div className="relative rounded-2xl border border-white/10 overflow-hidden" style={{background: "rgba(255,255,255,0.03)"}}>
            {/* Browser bar */}
            <div className="flex items-center gap-2 px-4 py-3 border-b border-white/5" style={{background: "rgba(255,255,255,0.02)"}}>
              ...
              <div className="flex-1 mx-3 h-6 rounded-md text-xs text-gray-600 flex items-center px-3 border border-white/5" style={{background: "rgba(255,255,255,0.03)"}}>
                tracktern-ten.vercel.app/{SCREENSHOTS[currentSlide].label.toLowerCase().replace(/ /g, "-")}
              </div>
              <span className="text-xs text-gray-600">{SCREENSHOTS[currentSlide].label}</span>
            </div>
            ...
            <div className="relative overflow-hidden" style={{aspectRatio: "16/9"}}>
              ...
              {/* Nav arrows */}
              <button onClick={() => goTo((currentSlide - 1 + SCREENSHOTS.length) % SCREENSHOTS.length)}
                className="absolute left-3 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full flex items-center justify-center border border-white/20 hover:bg-white/20 transition-all"
                style={{background: "rgba(0,0,0,0.5)"}}>
                <ChevronLeft className="w-4 h-4 text-white" />
              </button>
              <button onClick={() => goTo((currentSlide + 1) % SCREENSHOTS.length)}
                className="absolute right-3 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full flex items-center justify-center border border-white/20 hover:bg-white/20 transition-all"
                style={{background: "rgba(0,0,0,0.5)"}}>
            ...
            <div className="flex justify-center gap-2 py-3 border-t border-white/5">
              {SCREENSHOTS.map((s, i) => (
                <button key={i} onClick={() => goTo(i)} className="flex items-center gap-1.5 group">
                  <div className={`h-1.5 rounded-full transition-all duration-300 ${i === currentSlide ? "w-6 bg-violet-500" : "w-1.5 bg-white/20 group-hover:bg-white/40"}`} />
                </button>
              ))}
            </div>
          </div>
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {[
              ...
              { icon: CheckCircle, color: "#a78bfa", title: "Daily Practice", tag: "Practice", desc: "Pattern-wise DSA sheets with 700+ problems across 22 topics. Track progress just like CodeHurdle." },
            ].map(f => (
              <div key={f.title} className="rounded-2xl border p-6 hover:scale-[1.01] transition-all duration-200 group"
                style={{background: `${f.color}08`, borderColor: `${f.color}30`}}>
