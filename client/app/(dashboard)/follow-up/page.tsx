...
export default function FollowUpPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="glass-card rounded-2xl border border-border p-4 sm:p-6 space-y-5">
        <div>
      ...
      {email && <div className="glass-card rounded-2xl border border-primary/20 p-6 space-y-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-col gap-2 sm:flex-row">
            <button onClick={generate} className="flex items-center justify-center gap-1.5 text-xs px-3 py-1.5 rounded-lg border border-border text-muted-foreground hover:text-foreground"><RefreshCw className="w-3 h-3" /> Regenerate</button>
            <button onClick={() => copy(`Subject: ${subject}\n\n${email}`)} className="flex items-center justify-center gap-1.5 text-xs px-3 py-1.5 rounded-lg border border-primary/30 text-primary hover:bg-primary/10">{copied ? <><Check className="w-3 h-3" /> Copied!</> : <><Copy className="w-3 h-3" /> Copy All</>}</button>
          </div>
        </div>
